# -*- coding: utf-8 -*-
import os,socket,struct,psutil

try:
	import fcntl
except:
	print "没有fcntl模块，这是windows"
	pass
...
def get_local_ip(platform_category):
	...
	if platform_category.__contains__('windows'):#如果是windows，请大家帮完成
		...
		try:
			...
			my_local_ip='请注意检查网卡在本机的名称，并在源代码中添加，目前尚未识别对外IP'
			print my_local_ip
		pass
	return my_local_ip
	pass

def create_a_listneing_socket(port):
	sock=socket.socket(socket.AF_INET, socket.SOCK_STREAM)#创建ipv4的TCPIP端口
